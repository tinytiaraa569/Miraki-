...
function detectCountry(req) {
  ...
  const cookie = String(req.headers.cookie || "").match(/(?:^|;\s*)sf_country=([A-Za-z]{2})/)
...
storefrontRoutes.get("/get-allproducts/:alias/variant-media", async (req, res, next) => {
  try {
    const payload = await getStorefrontVariantMedia(req.params.alias, req.query.id, detectCountry(req))
...
storefrontRoutes.get("/resolve-product/:alias", async (req, res, next) => {
  try {
    const payload = await resolveStorefrontProductId(req.params.alias, detectCountry(req))
    if (!payload) return res.status(404).json({ error: "Product not found" })
    res.set("Cache-Control", "public, max-age=60, stale-while-revalidate=300")
    res.set("Vary", "Cookie, x-vercel-ip-country, cf-ipcountry")
    res.json(payload)
...
storefrontRoutes.get("/get-allproducts/:idOrAlias", async (req, res, next) => {
  try {
    ...
    const payload = await getStorefrontProduct({ alias: seg, id: seg }, detectCountry(req))
    if (!payload) return res.status(404).json({ error: "Product not found" })
    res.set("Cache-Control", "public, max-age=60, stale-while-revalidate=300")
    res.set("Vary", "Cookie, x-vercel-ip-country, cf-ipcountry")
    res.json(payload)
