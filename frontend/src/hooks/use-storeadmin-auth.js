...
export function useStoreAdminAuth() {
  const { data, error, isLoading, mutate } = useSWR("/seller/store-admins/auth/me", fetcher, {
  ...
  return {
    ...
    login: async (email, password) => {
      const res = await api.post("/seller/store-admins/auth/login", { email, password })
    ...
    verify2fa: async (code) => {
      await api.post("/seller/store-admins/auth/2fa/verify", { code })
      await mutate()
    },
    logout: async () => {
      ...
      try {
        await api.post("/seller/store-admins/auth/logout")
      } catch {
