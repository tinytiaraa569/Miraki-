...
import { ChevronLeft, ChevronRight, ChevronsUpDown, ChevronDown, ChevronUp, SlidersHorizontal } from "lucide-react"
...
import { buildOptionSetColumns } from "@/components/hub/option-sets/option-set-columns"
import { cn } from "@/lib/utils"

const PAGE_SIZES = [10, 25, 50, 100]

/**
 * Server-driven Option Sets list table (§10.2), rendered with plain React — NO
 * TanStack. Pagination, sorting and filtering all happen on the server via the
 * §6 `$facet` aggregation, so `data`/`pageCount` come straight off the SWR
 * envelope and this component only reflects that state and emits changes back up
 * through `setPagination` / `setSorting`.
 */
export function OptionSetTable({
  ...
  // so a deep page of the old result set can't 404 against the new one (§6.5).
            ...
            {showSkeleton ? (
                ...
                <TableCell colSpan={columns.length} className="h-48">
                  <div className="flex flex-col items-center gap-2 text-center">
                    ...
                    <p className="max-w-xs text-sm text-muted-foreground">
                      {showDeleted ? "There are no deleted option sets." : "Add an option set to get started."}
                    </p>
                  </div>
                </TableCell>
              </TableRow>
              ...
              rows.map((node) => (
                <TableRow
                  ...
                  onClick={(e) => {
                    // Ignore clicks that originate from the actions menu.
          ...
          {onToggleDeleted ? (
            <div className="flex items-center gap-2">
              <Switch
                ...
                aria-label="Show deleted option sets"
              />
              <Label htmlFor="os-show-deleted" className="text-sm font-normal text-muted-foreground">
                Show Deleted
              </Label>
            </div>
          ) : null}
